...
def train(
    ...
    # predefine model
    netG = CondWaveGANGenerator(**generator_config).to(device)
    netD = CondWaveGANDiscriminator(**discriminator_config).to(device)
    net = (netG, netD)
    ...
    for epoch in tqdm(range(n_epoch, n_epochs)):
        ...
        for i, batch in tqdm(enumerate(dataloader, 0)):
            ...
            signal_batch = batch[0].cuda().float()
            label_batch = batch[1].cuda().float()
            ...
            gradient_penalty = calc_gradient_penalty(netD, signal_batch,
            ...
            if train_G_flag:
                ...
                fake = netG(noise_Var, label_batch.cuda())
                G = netD(fake)
                G = G.mean()
...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--config', type=str, default='configs/config_cond_wavegan_star_ECG.json',
